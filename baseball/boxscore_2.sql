...
DROP TABLE IF EXISTS team_results_fix;
...
CREATE UNIQUE INDEX bf_game_team ON bp_factor (game_id, home_away);

DROP TABLE IF EXISTS bp_factor_prep;

CREATE TABLE bp_factor_prep
    SELECT
        bp.game_id,
        bp.bp_factor
    FROM bp_factor bp
    WHERE bp.home_away = "H"
    ORDER BY bp.game_id;

CREATE UNIQUE INDEX bf_game ON bp_factor_prep (game_id);
...
ADD gametime VARCHAR(10);

UPDATE boxscore_new bxn
SET bxn.month = MONTHNAME(DATE(game_date)),
    bxn.weekday = DAYNAME(DATE(game_date)),
...
SELECT * 
FROM boxscore_new bxn 
LEFT JOIN pythag_prep pp USING (game_id)
LEFT JOIN bp_factor_prep bf USING (game_id)
LEFT JOIN team_off_stats_prep tosp USING (game_id)
LEFT JOIN start_stats_prep ssp USING (game_id)
LEFT JOIN pen_stats_prep psp USING (game_id)
LEFT JOIN def_prep dp USING (game_id)
LEFT JOIN team_rest_prep trp USING (game_id)
LEFT JOIN cleanup_prep cp USING (game_id);
...
RENAME TABLE temp TO baseball_features;